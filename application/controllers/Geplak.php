...
class Geplak extends CI_Controller
{
    var $limit=10;
	var $offset=10;
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('m_geplak');
        $this->load->helper(array('url'));
    }

	function index($page=NULL,$offset='',$key=NULL)
	{       
        $data['query'] = $this->m_geplak->get_allimage();
        $this->load->view('components/header');
        $this->load->view('user/geplak/index',$data);
        $this->load->view('components/footer');
    }

    function add()
    {
	    $this->load->view('user/geplak/fupload');   
    }
    ...
    function insert(){
        ...
        $config['upload_path'] = './assets/images/geplak/'; //path folder
     ...
     public function insert_rasa(){
        ...
        if($_FILES['gambar']['name'])
        {
            if ($this->upload->do_upload('gambar'))
            {
                $gbr = $this->upload->data();
                $data = array(
                    'id_geplak' =>$this->input->post('id_geplak'),
                ...
                $this->m_geplak->get_insertrasa($data);
