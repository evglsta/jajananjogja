...
class Yangko extends CI_Controller
{
    var $limit=10;
	var $offset=10;
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('m_yangko');
        $this->load->helper(array('url'));
    }

	function index($page=NULL,$offset='',$key=NULL)
	{       
        $data['query'] = $this->m_yangko->get_allimage();
        $this->load->view('components/header');
        $this->load->view('user/yangko/index',$data);
        $this->load->view('components/footer');
    }

    function add()
    {
	    $this->load->view('user/yangko/fupload');   
    }
    ...
    function insert(){
        ...
        $config['upload_path'] = './assets/images/yangko/'; //path folder
     ...
     public function insert_rasa(){
        ...
        if($_FILES['gambar']['name'])
        {
            if ($this->upload->do_upload('gambar'))
            {
                $gbr = $this->upload->data();
                $data = array(
                    'id_yangko' =>$this->input->post('id_yangko'),
                ...
                $this->m_yangko->get_insertrasa($data);
