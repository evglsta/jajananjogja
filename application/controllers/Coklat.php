...
class Coklat extends CI_Controller
{
    var $limit=10;
	var $offset=10;
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('m_coklat');
        $this->load->helper(array('url'));
    }

	function index($page=NULL,$offset='',$key=NULL)
	{       
        $data['query'] = $this->m_coklat->get_allimage();
        $this->load->view('components/header');
        $this->load->view('user/coklat/index',$data);
        $this->load->view('components/footer');
    }

    function add()
    {
	    $this->load->view('user/coklat/fupload');   
    }
    ...
    function insert(){
        ...
        $config['upload_path'] = './assets/images/coklat/'; //path folder
     ...
     public function insert_rasa(){
        ...
        if($_FILES['gambar']['name'])
        {
            if ($this->upload->do_upload('gambar'))
            {
                $gbr = $this->upload->data();
                $data = array(
                    'id_coklat' =>$this->input->post('id_coklat'),
                ...
                $this->m_coklat->get_insertrasa($data);
